...
void declarations()
{
	...
	{
		printf(" in dec; missing at row=%d col=%d",tkn.row,tkn.col);
		exit(1);
	}
}
void datatype()
{
	...
	{
		printf(" in dt%s Missing datatype at row=%d col=%d",tkn.lexeme, tkn.row,tkn.col);
		exit(1);
	}
}
void idlist()
{
	...
	{
		printf("in idl Missing identifier at row=%d col=%d",tkn.row,tkn.col);
	}
}
void idlistprime()
{
...
void assignstat()
{
	if(strcmp(tkn.type,"identifier")==0)
	{
		...
		{
			printf("in as = missing at row=%d col=%d",tkn.row,tkn.col);
			exit(1);
		}
	}
	else
	{
		printf("in as Missing identifier at row=%d col=%d",tkn.row,tkn.col);
		exit(1);
	}
}
void expn()
